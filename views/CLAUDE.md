...
1. **ขอ access token ใหม่เสมอ** จาก `channel_id + secret` แล้วบันทึกกลับลง DB
2. `PUT /v2/bot/channel/webhook/endpoint` ตั้งเป็น `${URL}/webhook/${prefix}/${4หลักท้าย}.bot`
3. อ่านกลับมายืนยัน + ให้ LINE ยิงทดสอบ

ถามยืนยันก่อนเสมอ เพราะเขียนทับค่าที่ตั้งไว้ฝั่ง LINE

> **ห้ามกลับไปเช็ค token เดิมก่อนแล้วใช้ต่อ** — เคยทำแล้วได้ผลแย่กว่า
> token ที่ "ยังไม่ 401" ไม่ได้แปลว่าตั้ง webhook ผ่าน
> วิธีที่ใช้แก้ไลน์หลุดได้จริงมาตลอดคือ "แก้ไขไลน์แล้วกดบันทึก" ซึ่งขอ token ใหม่ทุกครั้ง
> ปุ่มนี้จึงต้องทำแบบเดียวกันเป๊ะ

### "LINE ยิงมาไม่ถึง" ไม่ใช่ความล้มเหลวของการตั้งค่า

ถ้า URL ตั้งตรงแล้วแต่ `webhook/test` timeout ส่วนใหญ่คือเซิร์ฟเวอร์กำลังตื่นจากหลับ
(Render free tier) ไม่ใช่ตั้งค่าผิด → **ไม่ติดธงแดง** และขึ้นข้อความแบบไม่ตกใจ
(`stillBad = webhook.matched === false` เท่านั้น ไม่รวม `delivery.ok`)

นี่คือสาเหตุที่ปุ่มนี้เคย "ดูแย่กว่า" การแก้ไข+บันทึก — เพราะการบันทึกไม่เคยทดสอบส่งเลย
เลยขึ้นเขียวเสมอ ทั้งที่สถานะจริงเหมือนกัน

### ขอ token ไม่ได้ ต้องบอกให้ตรงเรื่อง

backend ส่ง `tokenFailed: true` มาด้วย หน้าเว็บจะใช้ `data.message` ตรงๆ
("ไม่สามารถขอ access token ได้ ไลน์นี้มีปัญหา กรุณาตรวจสอบไลน์นี้")
ไม่ใช่ไปพูดเรื่อง "ตั้ง Webhook ไม่สำเร็จ" ซึ่งไม่ตรงกับสิ่งที่เกิดขึ้น
...
> **ปุ่มลอย "เพิ่มร้านค้า" อยู่มุมล่างกลางที่เดียวกัน** — `showLineToast()` จึงใส่
> `body.line-toast-open` ไว้ซ่อนปุ่มระหว่างที่ข้อความโชว์อยู่
> ปิดข้อความเมื่อไหร่ (หมดเวลา หรือกดที่กล่อง) ต้องผ่าน `hideLineToast()` เสมอ
> ไม่งั้น class ค้างแล้วปุ่มหายถาวร
